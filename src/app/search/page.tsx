...
function SearchContent() {
        ...
        if (res.data.success) {
          ...
          if (f.minRating > 0) {
            normalized = normalized.filter(
              (p) => (parseFloat(String(p.rating)) || 0) >= f.minRating,
            );
          }
  ...
  // Infinite scroll
  useEffect(() => {
    if (isClient && inView && hasMore && !loading) {
      const t = setTimeout(() => fetchProducts(page + 1), 200);
      return () => clearTimeout(t);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [inView, hasMore, loading, page, isClient]);
              ...
              return (
                <div key={cat.id}>
                  <div className="flex items-center">
                    ...
                    >
                      <span
                        className={`w-3.5 h-3.5 rounded border-2 flex items-center justify-center shrink-0 transition-all ${isSel ? "border-primary bg-primary" : "border-gray-300"}`}
                      >
                        {isSel && (
                          <svg
                            viewBox="0 0 10 8"
                        ...
                        <div className="ml-4 border-l border-gray-100 pl-2 py-0.5 space-y-0.5">
                          {cat.children!.map((child) => {
                            ...
                            return (
                              ...
                              >
                                <span
                                  className={`w-3 h-3 rounded border-2 flex items-center justify-center shrink-0 transition-all ${childSel ? "border-primary bg-primary" : "border-gray-300"}`}
                                >
                                  {childSel && (
                                    <svg
                                      viewBox="0 0 10 8"
          ...
          {AVAILABILITY_OPTIONS.map((opt) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-2.5 px-2 py-1.5 rounded-md text-sm text-left transition-all ${active ? "bg-primary/10 text-primary font-semibold" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"}`}
              >
                <span
                  className={`w-3.5 h-3.5 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${active ? "border-primary bg-primary" : "border-gray-300"}`}
                >
                  {active && (
        ...
        <div className="space-y-1">
          {RATING_OPTIONS.map((r) => (
            <button
              ...
              className={`w-full flex items-center gap-2 px-2 py-1.5 rounded-md text-sm transition-all ${minRating === r ? "bg-primary/10 text-primary font-semibold" : "text-gray-600 hover:bg-gray-50"}`}
            >
              <div className="flex">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              ...
              >
                ...
                <ChevronDown
                  className={`w-3.5 h-3.5 transition-transform shrink-0 ${showSort ? "rotate-180" : ""}`}
                />
              </button>
              <AnimatePresence>
                {showSort && (
                    ...
                    >
                      {SORT_OPTIONS.map((opt, i) => (
                        <button
                          ...
                          className={`w-full text-left px-4 py-2.5 text-sm transition-colors ${sortBy === opt.value ? "bg-primary/5 text-primary font-semibold" : "text-gray-700 hover:bg-gray-50"} ${i < SORT_OPTIONS.length - 1 ? "border-b border-gray-50" : ""}`}
                          disabled={loading}
                        >
                          {opt.label}
                        </button>
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm sticky top-24 overflow-hidden">
              ...
              {query && (
                <div className="px-4 pb-4">
                  ...
                  >
                    <RotateCcw className="w-3.5 h-3.5" /> Clear search & show
                    all
                  </button>
                </div>
              )}
