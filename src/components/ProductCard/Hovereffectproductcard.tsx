...
/**
 * HOVER-EFFECT layout — image fills the card entirely.
 * On hover: image zooms, a gradient panel slides up from the bottom
 * revealing price + action buttons. Clean and magazine-like.
 * Best for fashion, lifestyle, visual-first catalogues.
 */
export function HoverEffectProductCard(
  ...
  return (
    ...
    >
      ...
      {/* Scrim — always visible at bottom, expands on hover */}
      ...
      {cfg.show_discount_badge && hasDiscount && (
        <span className="absolute top-2.5 right-2.5 z-20 text-[10px] font-extrabold bg-red-500 text-white px-1.5 py-0.5 rounded-md">
          -{discountPct}%
        </span>
      )}
      ...
      {/* Slide-up panel */}
      <div
        className={`absolute inset-x-0 bottom-0 z-20 px-3 pb-3 transition-all duration-400 ease-out ${isMobile ? "translate-y-0" : "translate-y-4 group-hover:translate-y-0"}`}
      >
        {/* Title + price */}
        <div className="mb-2">
          ...
          >
            {cfg.show_add_to_cart && (
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-[11px] font-semibold transition-all active:scale-95 disabled:opacity-60 backdrop-blur-sm ${isAddedToCart ? "bg-green-500 text-white" : "bg-white text-gray-900 hover:bg-white/90"}`}
              >
                {cartLoading ? (
            ...
            {cfg.show_buy_now && (
              <button
                ...
                className="shrink-0 px-3 py-2 rounded-lg bg-primary text-white text-[11px] font-semibold transition-all active:scale-95 hover:bg-primary/90 disabled:opacity-60 backdrop-blur-sm"
              >
