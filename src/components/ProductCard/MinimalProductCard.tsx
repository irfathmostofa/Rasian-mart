...
import { CardConfig, ProductCardProps, WhatsAppConfig } from "@/types/ProductCard";
    ...
    if (isAddedToCart && cfg.primary_button === "add_to_cart") {
      return (
        <button
          ...
          disabled
        >
          <Check className={`${buttonSizes.icon} shrink-0`} />
    ...
    switch (cfg.primary_button) {
      case "buy_now":
        return (
          ...
          >
            {cartLoading ? (
              <span
                className={`${buttonSizes.icon} border-2 border-white border-t-transparent rounded-full animate-spin shrink-0`}
              />
            ) : (
              effectiveShowIcon && (
                <Zap className={`${buttonSizes.icon} shrink-0`} />
              )
            )}
      ...
      case "whatsapp":
        return (
          ...
          >
            {effectiveShowIcon && (
              <MessageCircle className={`${buttonSizes.icon} shrink-0`} />
            )}
        ...
        return (
          ...
          >
            {cartLoading ? (
              ...
              effectiveShowIcon && (
                <ShoppingCart className={`${buttonSizes.icon} shrink-0`} />
              )
            )}
  ...
  return (
    <>
      ...
      >
        {/* ══ IMAGE AREA ══════════════════════════════════════════════════════ */}
        <div
          className={`relative ${aspectClass} overflow-hidden bg-gray-50 shrink-0`}
        >
          <Link
            href={`/product/${slug}`}
        ...
        <div className="flex flex-col flex-1 p-2 sm:p-3 gap-1">
          ...
          {cfg.show_sku && (
            <p className="flex items-center gap-1 text-[8px] sm:text-[10px] text-gray-400">
              <Barcode className="w-2.5 h-2.5 sm:w-3 sm:h-3 shrink-0" />
            ...
            >
              ...
              {cfg.show_buy_now &&
                ...
                !isOutOfStock && (
                  <button
                    ...
                    className={`shrink-0 flex items-center justify-center rounded-lg border border-gray-200 text-gray-600 hover:border-primary hover:text-primary hover:bg-primary/5 transition-all ${iconPad} disabled:opacity-50`}
                  >
              ...
              {cfg.show_wishlist && (
                <button
                  ...
                  className={`shrink-0 flex items-center justify-center rounded-lg border transition-all ${iconPad} ${
                    isWishlisted
              ...
              {cfg.show_contact_whatsapp && !isOutOfStock && (
                <button
                  ...
                  className={`shrink-0 flex items-center justify-center rounded-lg text-white transition-opacity hover:opacity-85 ${iconPad}`}
                  style={{ background: cfg.whatsapp.button_color }}
                >
