"use client";

import Link from "next/link";
import { ChevronDown, ChevronRight } from "lucide-react";

interface Category {
  id: number;
  name: string;
  slug: string;
  children?: Category[];
}

const categories: Category[] = [
  {
    id: 1,
    name: "Men",
    slug: "men",
    children: [
      {
        id: 2,
        name: "Clothing",
        slug: "men-clothing",
        children: [
          { id: 3, name: "Shirts", slug: "men-shirts" },
          { id: 4, name: "Pants", slug: "men-pants" },
          { id: 19, name: "Jackets", slug: "men-jackets" },
          { id: 20, name: "Shoes", slug: "men-shoes" },
        ],
      },
      {
        id: 5,
        name: "Accessories",
        slug: "men-accessories",
        children: [
          { id: 21, name: "Watches", slug: "men-watches" },
          { id: 22, name: "Bags", slug: "men-bags" },
        ],
      },
    ],
  },
  {
    id: 6,
    name: "Women",
    slug: "women",
    children: [
      {
        id: 7,
        name: "Clothing",
        slug: "women-clothing",
        children: [
          { id: 8, name: "Dresses", slug: "women-dresses" },
          { id: 9, name: "Tops", slug: "women-tops" },
        ],
      },
      {
        id: 10,
        name: "Jewelry",
        slug: "women-jewelry",
        children: [
          { id: 25, name: "Necklaces", slug: "women-necklaces" },
          { id: 26, name: "Earrings", slug: "women-earrings" },
        ],
      },
    ],
  },
  { id: 29, name: "Beauty", slug: "beauty" },
  { id: 30, name: "Electronics", slug: "electronics" },
];

interface Props {
  mobile?: boolean;
}

export default function CategoryNav({ mobile = false }: Props) {
  if (mobile) {
    // Mobile: collapsible <details> menu
    return (
      <div className="space-y-1">
        {categories.map((cat) => (
          <details key={cat.id} className="group border-b">
            <summary className="flex justify-between items-center px-2 py-3 cursor-pointer hover:bg-gray-50 transition-colors">
              {cat.name}
              {cat.children && (
                <ChevronDown
                  className="transition-transform group-open:rotate-180"
                  size={16}
                />
              )}
            </summary>
            {cat.children && (
              <div className="pl-4 pb-2 space-y-1">
                {cat.children.map((sub) => (
                  <details key={sub.id} className="group">
                    <summary className="flex justify-between items-center px-2 py-2 cursor-pointer hover:bg-gray-50 transition-colors">
                      {sub.name}
                      {sub.children && (
                        <ChevronDown
                          className="transition-transform group-open:rotate-180"
                          size={14}
                        />
                      )}
                    </summary>
                    {sub.children && (
                      <div className="pl-4 pb-2 space-y-1">
                        {sub.children.map((child) => (
                          <Link
                            key={child.id}
                            href={`/category/${child.slug}`}
                            className="block px-2 py-2 hover:bg-gray-100 transition-colors"
                          >
                            {child.name}
                          </Link>
                        ))}
                      </div>
                    )}
                  </details>
                ))}
              </div>
            )}
          </details>
        ))}
      </div>
    );
  }

  // Desktop: hover dropdown menu
  return (
    <nav className="bg-gray-50 border-t">
      <div className="container mx-auto px-4 py-2 flex justify-between items-center">
        <div className="hidden md:flex gap-6 text-sm font-medium relative">
          {categories.map((cat) => (
            <div key={cat.id} className="relative group">
              <span className="flex items-center gap-1 hover:text-primary cursor-pointer transition-colors">
                {cat.name}
                {cat.children && (
                  <ChevronDown className="w-4 h-4 transition-transform group-hover:rotate-180" />
                )}
              </span>

              {cat.children && (
                <div className="absolute left-0 top-full bg-white shadow-lg border rounded-md mt-2 min-w-[200px] z-50 opacity-0 invisible group-hover:visible group-hover:opacity-100 transition-all duration-200">
                  {cat.children.map((sub) => (
                    <div key={sub.id} className="relative group/sub">
                      <Link
                        href={`/category/${sub.slug}`}
                        className="block px-4 py-2 hover:bg-gray-100 transition-colors"
                      >
                        {sub.name}
                      </Link>

                      {sub.children && (
                        <ChevronRight className="w-4 h-4 absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 group-hover/sub:text-primary" />
                      )}

                      {sub.children && (
                        <div className="absolute left-full top-0 bg-white shadow-lg border rounded-md min-w-[200px] hidden group-hover/sub:block">
                          {sub.children.map((child) => (
                            <Link
                              key={child.id}
                              href={`/category/${child.slug}`}
                              className="block px-4 py-2 hover:bg-gray-100 transition-colors"
                            >
                              {child.name}
                            </Link>
                          ))}
                        </div>
                      )}
                    </div>
                  ))}
                </div>
              )}
            </div>
          ))}
        </div>
      </div>
    </nav>
  );
}
